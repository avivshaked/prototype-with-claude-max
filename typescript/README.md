...
> ⚠️ **The auth method here is prototyping-only — not the SDK.** The Agent SDK itself is a production-grade SDK (Anthropic ships it for real agentic apps). What's personal-use-only is the `CLAUDE_CODE_OAUTH_TOKEN` / subscription auth method — every request bills against one person's subscription, which doesn't scale to multi-user traffic and may violate Anthropic's terms. To go to production, keep the Agent SDK if you're building agents (or switch to the regular [`@anthropic-ai/sdk`](https://www.npmjs.com/package/@anthropic-ai/sdk) package for direct LLM calls), and swap the auth to `ANTHROPIC_API_KEY` from [console.anthropic.com](https://console.anthropic.com). See the [top-level README](../README.md#%EF%B8%8F-the-auth-method-here-is-prototyping-only--not-the-sdk) for the full reasoning.
